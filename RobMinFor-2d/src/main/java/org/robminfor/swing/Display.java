...
public class Display {
	...
	public Display() {
		...
		mntmExit.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent arg0) {
				// note you will need to kill any other existing threads here
				// too
		...
		depthBar.addAdjustmentListener(new AdjustmentListener() {
			...
			public void adjustmentValueChanged(AdjustmentEvent e) {
				...
				// TODO consider not setting it if part of an ongoing value
				// change (slide)
			}
		});
		...
		controlpanel.setBorder(new EtchedBorder(EtchedBorder.LOWERED, null,
				null));
		...
		btnCreate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				JDialog dlg = new BuildBlockDialog(frame, jpanellandscape
						.getSelected(), landscape);
				dlg.pack();
				dlg.setLocationRelativeTo(frame);
				dlg.setVisible(true);
			}
						...
						updateworker = new SwingWorker<Void, Void>() {
							protected Void doInBackground() throws Exception {
								...
								try {
									...
									landscapetime.setText("" + hour + ":"
											+ minute);
									landscapedate.setText("" + day + " / "
											+ month + " / " + year);
									landscapemoney.setText("$"
											+ landscape.getMoney());
				...
				if (refreshworker == null || refreshworker.isDone()) {
					refreshworker = new SwingWorker<Void, Void>() {
						protected Void doInBackground() throws Exception {
							...
							float updatefraction = ((float) updateinterval / (float) updatetimer
									.getDelay());
