...
public class LandscapeFactory {
	...
	public Landscape generate(int sizex, int sizey, int sizez, int seed) {
		...
		IBlock ore = EntityManager.getEntityManager().getBlock("Ore");
		IBlock crystal = EntityManager.getEntityManager().getBlock("Crystal");
		IBlock air = EntityManager.getEntityManager().getBlock("Air");
		...
		for (int z = 0; z < surfaceheight; z++) {
			for (int y = 0; y < sizey; y++) {
				for (int x = 0; x < sizex; x++) {
					landscape.getSite(x, y, z).setBlock(air);
		...
		// not a percentage
		// assume a mean of 0.5 and a stddev of 0.1 (reasonable based on
		// sampling)
		double target = 0.80;
		for (int z = surfaceheight; z < sizez; z++) {
			for (int y = 0; y < sizey; y++) {
				for (int x = 0; x < sizex; x++) {
					...
					double oreValue = oreNoise.noise(dx / 10.0, dy / 10.0,
							dz / 5.0);
					double crystalValue = crystalNoise.noise(dx / 10.0,
							dy / 10.0, dz / 5.0);
					...
					if (oreValue > target && oreValue > crystalValue) {
						landscape.getSite(x, y, z).setBlock(ore);
						log.info("Ore!");
					} else if (crystalValue > target && crystalValue > oreValue) {
						landscape.getSite(x, y, z).setBlock(crystal);
						log.info("Crystal!");
					}
				}
			}
		...
		Site homesite = landscape.getSite(sizex / 2, sizey / 2,
				surfaceheight - 1);
		homesite.setBlock(EntityManager.getEntityManager().getBlock("Home"));
		homesite.setVisible();
		for (Site other : homesite.getAdjacents()) {
			if (!other.isSolid()) {
				other.setVisible(); // this will recurse as needed to other
									// sites
			}
		}
		...
		Agent agent;
		agent = new Agent(landscape.getSite((sizex / 2) + 1, sizey / 2,
				surfaceheight - 1));
		landscape.addAgent(agent);
		agent = new Agent(landscape.getSite((sizex / 2) - 1, sizey / 2,
				surfaceheight - 1));
		landscape.addAgent(agent);
		agent = new Agent(landscape.getSite(sizex / 2, (sizey / 2) + 1,
				surfaceheight - 1));
		landscape.addAgent(agent);
		agent = new Agent(landscape.getSite(sizex / 2, (sizey / 2) - 1,
				surfaceheight - 1));
